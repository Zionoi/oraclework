/*
    * TRIGGER
        ...
        EX)
        회원탈퇴시 회원테이블에서는 DELETE를 할 때, 탈퇴 회원들만 보관하는 테이블에 자동으로 INSERT처리
        만약 신고횟수가 일정 수를 넘으면 자동으로 해당 회원을 블랙리스트로 처리
        입출고에서 INSERT될때 마다 해당 상품의 재고수량이 UPDATE를 해야될 때 등등
        ...
        *트리거의 종류
            - SQL문 실행시기에 따른 분류
             ...
             >AFTER TRIGGER : 명시된 테이블에 이벤트가 발생된 후 트리거 실행
...
*/
-- EMPLOYEE테이블에 INSERT가 발생하고난 후 환영인사 트리거 
SET SERVEROUTPUT ON;
...
VALUES(223, '강남','021010-3156498','J1');

INSERT INTO EMPLOYEE(EMP_ID,EMP_NAME,EMP_NO,JOB_CODE) 
VALUES(224, '강동','021010-4556518','J2');
...
CREATE TABLE TB_PROSTOCK (
    ...
    PCODE NUMBER REFERENCES TB_PRODUCT,     --상품번호, 외래키 걸어둠
    TDATE DATE,                             --입고날짜
...
--205번 15개 판매
INSERT INTO TB_PROSALE
VALUES(SEQ_SCODE.NEXTVAL, 205, SYSDATE,15,2200000);  
--205번 상품이 13개 남아있는 상태에서 15개 판매하려 함
--설정한 오류 트리거 발생. 재고수량 부족 출력후 INSERT 실패
